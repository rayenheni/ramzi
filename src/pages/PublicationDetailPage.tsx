...
import { SmartImage, ImageLightbox } from '../components/shared';
import { ArrowLeft, ArrowRight, ShoppingBag, CheckCircle2, ShieldCheck, Tag } from 'lucide-react';
import { getIcon } from '../lib/icons';

export default function PublicationDetailPage({ publicationId }: { publicationId: string }) {
  const { content, setContent, lang } = useContent();
  const isAr = lang === 'ar';
  ...
  // Localized publication fields (Arabic with French fallback)
  const pubType = isAr && publication.typeAr ? publication.typeAr : publication.type;
  const pubMeta = isAr && publication.metaAr ? publication.metaAr : publication.meta;
  const pubTitle = isAr && publication.titleAr ? publication.titleAr : publication.title;
  const pubDescription = isAr && publication.descriptionAr ? publication.descriptionAr : publication.description;
  const pubFullContent = isAr && publication.fullContentAr ? publication.fullContentAr : publication.fullContent;
    ...
    <div className="pt-32 pb-24 bg-paper min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="mb-8">
          ...
          >
            <BackIcon size={16} />
            {isAr ? 'العودة إلى جميع المنشورات' : 'Retour à toutes les publications'}
          </a>
        </div>
          ...
          <div className="mb-10 p-6 bg-emerald-900/10 border-2 border-emerald-600 text-emerald-950 rounded-md flex items-start gap-4 shadow-sm animate-fade-in">
            ...
            <div className="space-y-1">
              <h4 className="font-serif text-lg font-semibold text-emerald-900">
                {isAr ? 'تم تسجيل طلب الحجز بنجاح!' : 'Demande de réservation enregistrée avec succès !'}
              </h4>
              <p className="text-sm text-emerald-800 leading-relaxed">
                {isAr ? (
                  <>تم إرسال طلب الحجز رقم <code className="font-mono font-bold">{reservationSuccess}</code> للمؤلف « {pubTitle} » إلى المكتب بنجاح. سيتصل بكم فريقنا قريبا على الهاتف المذكور لتأكيد التسليم.</>
                ) : (
              ...
              >
                {isAr ? 'إغلاق هذه الرسالة' : 'Fermer ce message'}
              </button>
            </div>
          </div>
        )}
            ...
            <div className="sticky top-32 space-y-6">
              {publication.coverImage ? (
                ...
                >
                  <SmartImage
                    ...
                    alt={pubTitle}
                    label={isAr ? 'غلاف المنشور' : 'Couverture de la publication'}
                    showZoomIcon={true}
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-ink/90 backdrop-blur-sm text-paper py-3 px-4 text-center text-xs tracking-wider uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    {isAr ? '🔍 تكبير الغلاف' : '🔍 Agrandir la couverture'}
                  </div>
                </div>
              ) : (
                <div className="w-full aspect-[3/4] bg-cream/60 border border-ink/10 flex flex-col items-center justify-center p-8 text-center text-ink/40 space-y-3">
                  ...
                  <span className="text-xs uppercase tracking-widest">{pubType}</span>
                </div>
              )}
              ...
              <div className="bg-cream/40 border border-ink/15 p-6 space-y-4 rounded-sm shadow-sm">
                <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-gold font-semibold">
                  <ShoppingBag size={16} />
                  {isAr ? 'التوفر والطلب' : 'Disponibilité & Commande'}
                </div>
                ...
                <p className="text-xs text-ink/70 leading-relaxed">
                  {isAr
                    ? 'احجزوا نسخة مباشرة لدى المكتب. التسليم يدا بيد أو التوصيل إلى كامل تراب الجمهورية.'
                ...
                >
                  ...
                  {isAr ? 'حجز / طلب نسخة' : 'Réserver / Commander un exemplaire'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-8">
              ...
              <div className="pt-2 flex flex-wrap gap-4 text-xs md:text-sm text-ink/70">
                {publication.isbn && (
                  <div className="bg-cream/60 px-4 py-2 border border-ink/10 flex items-center gap-2">
                    ...
                    <span><strong className="font-medium text-ink">{isAr ? 'ردمك :' : 'ISBN :'}</strong> {publication.isbn}</span>
                  </div>
                )}
                {publication.price && (
                  <div className="bg-cream/60 px-4 py-2 border border-ink/10 flex items-center gap-2">
                    <ShoppingBag size={14} className="text-gold" />
                    <span><strong className="font-medium text-ink">{isAr ? 'الثمن :' : 'Prix :'}</strong> {publication.price}</span>
                  </div>
                )}
                <div className="bg-cream/60 px-4 py-2 border border-ink/10 flex items-center gap-2">
                  <ShieldCheck size={14} className="text-gold" />
                  <span><strong className="font-medium text-ink">{isAr ? 'المؤلف :' : 'Auteur :'}</strong> {isAr ? 'الأستاذ' : 'Me'} {isAr ? `${content.heroFirstNameAr || content.heroFirstName} ${content.heroLastNameAr || content.heroLastName}` : `${content.heroFirstName} ${content.heroLastName}`}</span>
                </div>
              </div>
            </div>
            ...
            {pubDescription && (
              <div className="space-y-3">
                <h3 className="text-xs uppercase tracking-[0.25em] text-gold font-semibold">
                  {isAr ? 'التقديم والملخص' : 'Présentation & Résumé'}
                </h3>
                <p className="text-lg md:text-xl text-ink/80 leading-relaxed font-serif italic border-s-2 border-gold ps-5 py-1">
                  {pubDescription}
                </p>
              </div>
            )}
            ...
            {pubFullContent && (
              <div className="space-y-4 pt-4">
                <h3 className="text-xs uppercase tracking-[0.25em] text-gold font-semibold">
                  {isAr ? 'مقتطف ومحتوى المؤلف' : "Extrait & Contenu de l'ouvrage"}
                </h3>
            ...
            <div className="pt-8 border-t border-ink/10 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div>
                <h4 className="font-serif text-lg text-ink">{isAr ? 'ترغبون في اقتناء هذا المؤلف؟' : 'Vous souhaitez vous procurer cet ouvrage ?'}</h4>
                <p className="text-sm text-ink/60">{isAr ? 'اتصلوا مباشرة بمكتب المحاماة أو احجزوا عبر الإنترنت.' : "Contactez directement le cabinet d'avocat ou effectuez une réservation en ligne."}</p>
              </div>
              <button
                onClick={() => setShowReservationModal(true)}
              ...
              >
                <ShoppingBag size={16} />
                {isAr ? 'طلب نسخة' : 'Commander un exemplaire'}
              </button>
            </div>
          </div>
        </div>
          ...
          items={[
            {
              src: publication.coverImage,
              alt: isAr
                ? `الغلاف الرسمي : ${pubTitle} — الأستاذ ${content.heroFirstNameAr || content.heroFirstName} ${content.heroLastNameAr || content.heroLastName}`
                : `Couverture officielle : ${publication.title} — Maître ${content.heroFirstName} ${content.heroLastName}`,
            },
          ]}
          currentIndex={0}
          ...
          <div className="bg-paper max-w-lg w-full p-6 md:p-8 border border-ink/20 shadow-2xl relative my-8">
            <div className="flex items-center justify-between border-b border-ink/10 pb-4 mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-serif text-xl font-bold text-ink">{isAr ? 'حجز مؤلف' : "Réservation d'ouvrage"}</h3>
                  <p className="text-xs text-ink/60 line-clamp-1">{pubTitle}</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleReservationSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={isAr ? 'مثال: الأستاذ كريم بن سالم' : 'Ex : Maître Karim Ben Salem'}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs uppercase tracking-wider text-ink/70 font-semibold mb-1">
                    {isAr ? 'عدد النسخ' : "Nombre d'exemplaires"}
                  </label>
                  <input
                    type="number"
                    min={1}
              ...
              <div>
                <label className="block text-xs uppercase tracking-wider text-ink/70 font-semibold mb-1">
                  {isAr ? 'عنوان التسليم / المدينة *' : 'Adresse de livraison / Ville *'}
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder={isAr ? 'مثال: شارع فرحات حشاد، سوسة' : 'Ex : Rue Farhat Hached, Sousse'}
                  className="w-full px-4 py-2.5 bg-cream/40 border border-ink/20 text-sm text-ink outline-none focus:border-gold"
                />
              </div>
              ...
              <div>
                <label className="block text-xs uppercase tracking-wider text-ink/70 font-semibold mb-1">
                  {isAr ? 'ملاحظات أو تعليمات' : 'Remarques ou instructions'}
                </label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder={isAr ? 'هل ترغبون في إهداء أو توصيل سريع؟' : 'Souhaitez-vous une dédicace ou une livraison urgente ?'}
                  className="w-full px-4 py-2.5 bg-cream/40 border border-ink/20 text-sm text-ink outline-none focus:border-gold"
                />
              </div>
